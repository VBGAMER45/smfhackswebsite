...
// End Ad Seller Pro Location - Inside First Post
		...
		// Start Awesome Post Ratings
		if ($modSettings['apr_enabled'] == 1)
		{
			global $sourcedir, $modSettings, $user_info;
			require_once($sourcedir . '/AwesomePostRatings2.php');

			echo '<div align="left" id="awemsgid' . $message['id'] . '">';
			// Get ratings on the post
			if (!empty($message['aprratings']))
			{
				$awe_ratings = Awesome_LoadMessagDisplay($message['id']);
				if (!empty($awe_ratings))
				{
					foreach($awe_ratings as $myrating)
					{
						echo  '<img src="' . $modSettings['apr_url'] . $myrating['icon'] . '" alt="' . $myrating['title'] . '" title="' . $myrating['title'] . '" />' . '&nbsp;' . $myrating['title'] . '&nbsp;x&nbsp;' . $myrating['total'] . '&nbsp;';
					}

 					echo '<a href="' . $scripturl . '?action=awesome;sa=viewratings;msgid=' . $message['id'] . '" onclick="return reqWin(this.href, 600, 400, false);">' . $txt['awepost_text_viewlist'] . '</a>';
				}
			}

			echo '</div>';

			// Show list of Rating options
			if ($user_info['is_guest'] == false && $message['member']['id'] != $user_info['id'] && allowedTo('apr_rate'))
			{
				echo '<div align="right">
				<span id="errrateawemsgid' . $message['id'] . '"></span>
				<span id="rateawemsgid' . $message['id'] . '">';


				foreach($context['awe_ratingpostslist'] as $ratechoice)
				{
					echo '<a href="#ratethis" onclick="return AweAjaxRatingAdd(' . $ratechoice['id_ratetype']  . ',' . $context['current_topic']  . ',' . $message['id'] . ');"><img src="' . $modSettings['apr_url'] . $ratechoice['icon'] . '" alt="' . $ratechoice['title'] . '" title="' . $ratechoice['title'] . '" class="awefade" /></a>';
				}
