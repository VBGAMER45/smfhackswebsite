...
function template_control_richedit($editor_id, $smileyContainer = null, $bbcContainer = null)
{
	...
	if ($modSettings['msg_editorpro_enable'] == 1)
	{
		template_control_sce_richedit($editor_id, $smileyContainer, $bbcContainer);
		return;
	}
...
<img src="' . $settings['default_theme_url'] . '/images/emoji.png" class="emoji-btn"/>
 </div>';
	// End emoji pro
...
function template_control_verification($verify_id, $display_type = 'all', $reset = false)
{
	...
	$total_items += ($verify_context['can_hcaptcha'] ? 1 : 0);
	...
	for ($i = 0; $i < $total_items; $i++)
	{
        ...
        if ($i == 0 && $verify_context['can_hcaptcha'])
        {
	...
	else if ($i == 0 && ($verify_context['show_visual'] || $modSettings['recaptcha_enabled']))
		{
		...
		else
		{
			...
			{
				$qIndex = $verify_context['can_hcaptcha'] ? $i - 1 : $i;
			}	

			if (!empty($verify_context['questions'][$qIndex]['q']))
			echo '
				<div class="smalltext">
...
function template_control_sce_richedit($editor_id, $smileyContainer = null, $bbcContainer = null)
{
 ...
 */
	...
	if (!empty($modSettings['msg_editorpro_height']))
		$editor_context['height'] = $modSettings['msg_editorpro_height'] . 'px';
	...
	echo '
		...
		<script>
			$(document).ready(function() {
				...
				sceditor.create(textarea, ', json_encode($editor_context['sce_options'], JSON_PRETTY_PRINT), ');', !$editor_context['sce_options']['emoticonsEnabled'] ? '' : '
